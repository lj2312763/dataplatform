...
<script>
    import operate from "@/components/operate/split";
    import {split} from "@/api/data";
        ...
        methods: {
            ...
            getCdata(data) {
                ...
                var params = {
                    ...
                    "splitChar": data.type[0],
                    "accessToken": localStorage.getItem('yk-token')
                };
                if (this.columnName === "") {
                    this.$message.error("请选择字段名称")
                } else {
                            ...
                            for (var k in res.data[0]) {
                                this.tableColumn.push({
                                    ...
                                    slots: {
                                        "header": ({column}) => {
                                            return [
                                                <span style="cursor: pointer" onClick={this.headerClickEvent}>{column.title}</span>
                                            ]
                                        },
                                    }
                                })
