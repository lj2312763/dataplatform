...
<script>
    import operate from "@/components/operate/replace";
    import {replace} from "@/api/data";
        ...
        methods: {
            ...
            getCdata(data) {
                ...
                if(data.type===2){
                    ...
                    data.strReplace=data.strReplace;
                }
                var params = {
                    "tableId": this.tableId,
                    "strParam": data.strParam,
                    "strReplace": data.strReplace,
                    "strFind": data.strFind,
                    /* "accessToken": localStorage.getItem('yk-token')*/
                };
                if(data.strParam==="id"||data.strParam==="strReplace"){
                    this.$message.error("id是主键不可操作")
                }else{
                    replace(params).then(res => {
                        console.log(res)
                        if(res.code===500000){
                            this.$message.error("该数据不能替换")
                        }else{
                            this.tableHead = [];
                            this.tableColumn = [];
